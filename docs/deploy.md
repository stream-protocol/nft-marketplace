...
```json
"deploy:gh": "yarn export && gh-pages -d ../../build/web --repo https://github.com/stream-protocol/nft-marketplace -t true",
"deploy": "cross-env ASSET_PREFIX=/nft-marketplace yarn build && yarn deploy:gh",
```

There are 2 things to change:

- specify your repo URL instead of `https://github.com/stream-protocol/nft-marketplace` (for example, `https://github.com/my-name/my-metaplex`)
